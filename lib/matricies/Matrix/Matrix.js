import { copyMatrix, addMatricies, emptyMatrix, 
		 ...
		 matrixDimensions, matrixToArray, multiplyMatricies, randomizeMatrix, 
...
export class Matrix {
	...
	multiply(matrix) {
		if(matrix instanceof Matrix) {
			this.matrix = multiplyMatricies(this.matrix, matrix.matrix);
		} else {
			this.matrix = multiplyMatricies(this.matrix, matrix);
		}
